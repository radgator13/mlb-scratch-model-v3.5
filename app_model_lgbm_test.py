...
st.set_page_config(page_title="MLB NRFI LightGBM Model Predictions", layout="wide")
...
@st.cache_resource
def load_model():
    return joblib.load("model_lgbm.pkl")
...
# Title
st.title("⚾ MLB NRFI Predictions (LightGBM Model)")
...
if view_option == "Predictions Only":
    st.subheader(f"LightGBM Model NRFI Predictions for {selected_date_str}")
    st.dataframe(
    ...
    st.subheader(f"LightGBM Model NRFI Predictions vs Actual Results for {selected_date_str}")
    st.dataframe(
...
# Footer
st.markdown("---")
st.caption("Created by ⚾ NRFI LightGBM Model Tester 2025")
