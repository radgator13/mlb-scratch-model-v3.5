...
st.set_page_config(page_title="MLB NRFI Random Forest Model", layout="wide")
...
# Title
st.title("⚾ MLB NRFI Predictions (Random Forest Model)")
...
if view_option == "Predictions Only":
    st.subheader(f"Random Forest Model NRFI Predictions for {selected_date_str}")
    st.dataframe(
    ...
    st.subheader(f"Random Forest Model NRFI Predictions vs Actual Results for {selected_date_str}")
    st.dataframe(
...
# Footer
st.markdown("---")
st.caption("Created by ⚾ NRFI Random Forest Tester 2025")
